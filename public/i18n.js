...
(function () {
  ...
  const D = {
    "zh-TW": {
      ...
      "install.req.lead": "需要 Node.js 20.19.0 LTS 以上或更新的 LTS 版本，且 node 與 npx 必須可在目前 shell 的 PATH 中執行。ask-bridge 會透過 npx 啟動版本固定的 chrome-devtools-mcp@1.5.0；Node.js 版本過舊時，MCP server 可能在 initialize 階段直接退出。",
    ...
    "zh-CN": {
      ...
      "install.req.lead": "需要 Node.js 20.19.0 LTS 以上或更新的 LTS 版本，并且 node 与 npx 必须能在当前 shell 的 PATH 中执行。ask-bridge 会通过 npx 启动固定版本的 chrome-devtools-mcp@1.5.0；Node.js 版本过旧时，MCP server 可能在 initialize 阶段直接退出。",
      "install.req.mac.title": "macOS",
    ...
    "ja": {
      ...
      "install.req.lead": "Node.js 20.19.0 LTS 以上、またはより新しい LTS が必要です。node と npx は現在の shell の PATH から実行できる必要があります。ask-bridge は npx 経由で固定版 chrome-devtools-mcp@1.5.0 を起動します。Node.js が古い場合、MCP server は initialize 段階で終了することがあります。",
      "install.req.mac.title": "macOS",
    ...
    "ko": {
      ...
      "install.req.lead": "Node.js 20.19.0 LTS 이상 또는 더 새로운 LTS 가 필요하며, node 와 npx 는 현재 shell 의 PATH 에서 실행 가능해야 합니다. ask-bridge 는 npx 로 고정된 chrome-devtools-mcp@1.5.0 을 시작합니다. Node.js 가 너무 오래되면 MCP server 가 initialize 단계에서 종료될 수 있습니다.",
      "install.req.mac.title": "macOS",
    ...
    "en": {
      ...
      "install.req.lead": "You need Node.js 20.19.0 LTS or a newer LTS, and both node and npx must be available in the current shell's PATH. ask-bridge starts the pinned chrome-devtools-mcp@1.5.0 through npx; older Node.js versions can cause the MCP server to exit during initialize.",
      "install.req.mac.title": "macOS",
